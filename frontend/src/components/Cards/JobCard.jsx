...
const JobCard = ({ job, onClick, onToggleSave, onApply, saved, hideApply }) => {
  ...
  // Convert salary from ₹ to Lakh and format
  const formatSalary = (min, max) => {
    const formatNumber = (num) => {
      if (!num) return ""
      return (Number(num) / 100000).toFixed(2) // converts to Lakh
    }
    if (!min && !max) return "N/A"
    if (min && max) return `${formatNumber(min)} - ${formatNumber(max)} Lakh`
    if (min) return `${formatNumber(min)} Lakh`
    if (max) return `${formatNumber(max)} Lakh`
  }
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-gray-900 text-base group-hover:text-teal-600 transition-colors leading-snug">
              {job?.title}
            </h3>
        ...
        {user && (
          ...
          >
            <Bookmark
              className={`w-5 h-5 hover:text-teal-600 ${
                job?.isSaved || saved ? "text-teal-600" : "text-gray-400"
              }`}
            />
          </button>
        )}
